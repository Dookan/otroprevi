...
<div class="card shadow mb-4">
	<div class="card-header py-2">
		<h6 class="m-0 font-weight-bold text-primary d-inline-block py-2">Pagos realizados</h6>
	</div>
	<div class="card-body">
			...
			<table class="table table-bordered text-center" id="dataTable" width="100%" cellspacing="0"
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<span class="btn btn-primary" id="openModal" data-toggle="modal"
								data-target="{{'#'."modal".$payment->id}}">Adjuntar</span>
						</td>
						@else
						...
						<td>
							<span class="btn btn-primary" id="openModal" data-toggle="modal"
								data-target="{{'#'."imgModal-".$payment->id}}">Ver</span>
						</td>
						@else
						<td>
							<a href="{{Storage::url($payment->bill)}}" class="btn btn-primary text-white"
								target="blank">Ver</a>
						</td>
						@endif
						@endif
					...
					<div id="modal-{{$payment->id}}" class="modal">
						<div class="card w-50 modalContent">
							<span class="d-block btn btn-danger ml-auto text-center mb-2" id="closeBtn"
								onclick="closeModal()" style="width: 40px;">X</span>
							<form action="/admin/update-payment/{{$payment->id}}" method="POST"
								enctype="multipart/form-data">
								@csrf
								<div class="form-group">
									<input type="hidden" name="_method" value="PUT">
									<input type="file" class="d-block mb-2 ml-5 @error('bill') is-invalid @enderror"
										name="bill" required>
									...
									<div class="card-body mb-4 ">
										<p class="text-danger"><strong>Nota: Solo se pueden adjuntar formatos de imagen
												y pdf.</strong></p>
									</div>
					...
					{{-- Modal Attach --}}
					<div class="modal fade text-center" id="{{"modal".$payment->id}}" tabindex="-1" role="dialog"
						aria-labelledby="attachModalLabel" aria-hidden="true">
						<div class="modal-dialog" role="document">
							<div class="modal-content">
								<div class="modal-header">
									<h5 class="modal-title" id="attachModalLabel">Adjuntar Comprobante</h5>
								...
								<div class="modal-body">
									...
									<form action="/admin/update-payment/{{$payment->id}}" method="POST"
										...
										<div>
											<input type="hidden" name="_method" value="PUT">
											<input type="file"
												class="d-block mb-2 ml-5 @error('bill') is-invalid @enderror"
												name="bill" required>
					...
					<div class="modal fade text-center" id="{{"imgModal-".$payment->id}}" tabindex="-1" role="dialog"
						...
						<div class="modal-dialog" role="document">
							<div class="modal-content">
								<div class="modal-header">
									<h5 class="modal-title" id="imgModalLabel">Mostrar comprobante</h5>
									<button class="close" type="button" data-dismiss="modal" aria-label="Close">
										<span aria-hidden="true">×</span>
									</button>
								</div>
