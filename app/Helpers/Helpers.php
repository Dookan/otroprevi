...
class Helper 
{
	...
	public static function policies_money_month(){
		...
		if ($policies->first() != null) {
			$prices = 0;
			foreach ($policies as $policy) {
				$prices = $prices + $policy->total_all;		
			}
	...
	public static function policies_month($month){
		$year = Carbon::now()->year;
		$policies = Policy::where('deleted_at', null)->whereMonth('created_at', $month)->whereYear('created_at', $year)->get();
...
// User graphics helpers
	public static function policies_valid_user(){
		...
		$policies = Policy::where('user_id', $user_id)->whereDate('expiring_date', '>=', $date)->get();
	...
	public static function policies_sold_weeks(){
		...
		$policies = Policy::where('user_id', $user_id)->whereDate('created_at','>=', $weeks)->whereDate('created_at', '<=', $today)->whereYear('created_at', $year)->get();
	...
	public static function policies_revenue_year(){
		...
		$policies = Policy::where('user_id', $user_id)->whereYear('created_at', $year)->get();
	...
	public static function policies_revenue_month(){
		$year = Carbon::now()->year;
		$month = Carbon::now()->month;
		$user_id = Auth::user()->id;

		$policies = Policy::where('deleted_at', null)->where('user_id', $user_id)->whereYear('created_at', $year)->whereMonth('created_at', $month)->get();
	...
	public static function policies_month_user($month){
		$user_id = Auth::user()->id;
		$year = Carbon::now()->year;
		$policies = Policy::where('deleted_at', null)->where('user_id', $user_id)->whereMonth('created_at', $month)->whereYear('created_at', $year)->get();
